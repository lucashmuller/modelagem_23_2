function I_nova = lambdap(I_sn, T, T_n, q, V_go, A, k, G, G_n, I_scpainel, K_i, V, R_s, R_p, lamb)
erro = 1; %Erro da função
I_nova = 1; %Valor de corrente assumido
Tol = 1*10^-6; %Tolerancia do resultado
...
while abs(erro)>Tol 
    ...
    I_nova = I_pvpainel -(I_s.*(exp((V+R_s.*I_antigo)./(V_t*A))-1)) - ((V+R_s.*I_antigo)./R_p); %Calcula o novo valor de corrente
